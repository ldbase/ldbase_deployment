...
shell_exec($db_auth_string . "CREATE USER '${db_user}'@'%' IDENTIFIED BY '${db_pass}';" . $db_end_string);
shell_exec($db_auth_string . "GRANT ALL PRIVILEGES ON ${db_name}.* TO '${db_user}'@'%';" . $db_end_string);
shell_exec($db_auth_string . "FLUSH PRIVILEGES;" . $db_end_string);
shell_exec('service mysql restart');
shell_exec('cd /var/www/html; composer create-project drupal-composer/drupal-project:8.x-dev drupal --stability dev --no-interaction >> /root/drupal-rebuild.txt 2>&1');
shell_exec("cd /var/www/html/drupal; /var/www/html/drupal/vendor/bin/drupal site:install standard --langcode=\"en\" --db-type=\"mysql\" --db-host=\"${db_host}\" --db-name=\"${db_name}\" --db-user=\"${db_user}\" --db-pass=\"${db_pass}\" --db-port=\"3306\" --site-mail=\"bjbrown@fsu.edu\" --account-name=\"admin\" --account-pass=\"${randpass}\" --account-mail=\"bjbrown@fsu.edu\" --no-interaction >> /root/drupal-rebuild.txt 2>&1");
